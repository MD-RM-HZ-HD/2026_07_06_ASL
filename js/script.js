...
text: {
    ...
    cards: {
        ...
        2: typeof Flash_Cards_1 !== 'undefined' ? Flash_Cards_1.map((item, i) => ({ _id: `c1_${i}`, q: formatContent(item.front), a: formatContent(item.back) })) : null
    },
    tf: {
    ...
    mcq: {
        ...
        3: typeof Multiple_Choice_2 !== 'undefined' ? Multiple_Choice_2.map((item, i) => ({ _id: `m2_${i}`, q: formatContent(item.q), opts: formatContent(item.options), correct: item.correctIndex, exp: formatContent(item.rationale) })) : null
    },
    fill: {
...
function updateTabCounters() {
    ...
    keys.forEach(key => {
        if (DB[key]) {
            ...
            const c3 = DB[key][3] ? DB[key][3].length : 0;       

            // جمع الأقسام التي تحتوي على أسئلة فقط
            let parts = [];
            if (c1 > 0) parts.push(c1);
            if (c2 > 0) parts.push(c2);
            if (c3 > 0) parts.push(c3);

            // دمج الأرقام بعلامة + إذا كان هناك أكثر من قسم، أو عرض الرقم الوحيد
            counts[key] = parts.length > 1 ? parts.join(' + ') : (parts[0] || 0);
        }
    });
